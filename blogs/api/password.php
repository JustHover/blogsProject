...
if(isset($_COOKIE['id']) && $_COOKIE['id']!=""){
	...
	if(isset($_POST['oldPassword']) && $_POST['oldPassword']!=""){
		...
		$sql = "SELECT * From user WHERE password = '$oldPassword' AND id=".$_COOKIE['id'];
    ...
    if ($valid) {
        ...
        $result = mysqli_query($conn,"SELECT * From user WHERE id =".$_COOKIE['id']);  //验证是否存在
        if(mysqli_num_rows($result) == 1 ){ 

          	$sql="UPDATE user SET password='$newPassword' WHERE id=".$_COOKIE['id'];
